...
export function getConsent(traineeId) {
  ...
  const events = data.eventsByTrainee[traineeId] || []
  return {
    ...consent,
...
export function listConsents() {
  ...
  return data.consents.map((c) => {
    ...
    const evs = raw.events.filter((e) => e.trainee_id === c.trainee_id)
    return {
      ...c,
      id: c.trainee_id,
      name: t?.name,
...
export function postCheckin(body = {}) {
  ...
  const prior = data.eventsByTrainee[body.trainee_id] || []
