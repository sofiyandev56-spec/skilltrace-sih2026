...
const emptyOverlay = () => ({
  ...
  extraTrainees: [],  // newly registered trainees
  seq: 0,
})
...
export function currentData() {
  ...
  const trainees = [...base.trainees, ...(overlay.extraTrainees || [])].filter((t) => !withdrawn[t.id])
...
export function registerNewTrainee(trainee, events) {
  initStore()
  overlay.extraTrainees = overlay.extraTrainees || []
  overlay.extraTrainees.push(trainee)
  if (events) {
    overlay.extraEvents.push(...events)
  }
  notify()
}
