...
export function classifyIntent(text, role = 'ministry') {
  ...
  // 3. Provider Comparison / Ranking Intents
  if (
  ...
  try {
    switch (intent.type) {
      case 'DASHBOARD_SUMMARY': {
        ...
        if (!data) return getTranslation(langKey, 'noDataAvailable')
      ...
      case 'TRAINEE_SEARCH': {
        ...
        if (!qId) {
          if (langKey === 'hi') return 'कृपया खोजने के लिए प्रशिक्षु आईडी प्रदान करें (जैसे T10245 या TRN000001)।'
          if (langKey === 'mr') return 'कृपया शोधण्यासाठी प्रशिक्षणार्थी आयडी प्रविष्ट करा (उदा. T10245 किंवा TRN000001).'
          return 'Please provide a valid trainee ID to search (e.g., T10245 or TRN000001).'
        }

        const result = await searchTrainee(qId, role, currentUserId)
        if (!result.authorized) {
          return getTranslation(langKey, 'clientRestricted')
        }
        if (!result.found) {
          return getTranslation(langKey, 'traineeNotFound', qId)
        }
      ...
      case 'PROVIDER_COMPARISON': {
        ...
        if (!providers.length) return getTranslation(langKey, 'noDataAvailable')
      ...
      case 'CLIENT_SKILL_GAP': {
        ...
        if (!clientData) return getTranslation(langKey, 'noDataAvailable')
      ...
      default: {
        return getTranslation(langKey, 'fallback', MOCK_HELPLINE_NUMBER)
      }
    }
  } catch (err) {
