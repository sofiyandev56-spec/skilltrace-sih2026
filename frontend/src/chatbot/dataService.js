...
/**
 * Adjudication queue. Ministry only: a dispute names two parties and holds
 * both their claims, so it is not a trainee's to read through the assistant.
 */
export async function getDisputesSummary(requesterRole) {
  if (requesterRole !== 'ministry') return null
  try {
    const disputes = (await api.getDisputes()) || []
    const open = disputes.filter((d) => d.status !== 'resolved')
    const resolved = disputes.filter((d) => d.status === 'resolved')
    const unassigned = open.filter((d) => !d.assigned_officer_id)
    return {
    ...
    console.error('Error in getDisputesSummary:', err)
    return null
  }
}

/** Follow-up queue: trainees unreachable after repeated contact attempts. */
export async function getFollowupSummary(requesterRole) {
  if (requesterRole !== 'ministry') return null
  try {
    const queue = (await api.getFollowupQueue()) || []
    const unassigned = queue.filter((q) => !q.assigned_to)
    const byChannel = {}
    queue.forEach((q) => {
      const key = q.channel || 'Unknown'
      byChannel[key] = (byChannel[key] || 0) + 1
    })
    return {
      total: queue.length,
      unassigned: unassigned.length,
      assigned: queue.length - unassigned.length,
      channels: byChannel,
    }
  } catch (err) {
    console.error('Error in getFollowupSummary:', err)
    return null
  }
}
